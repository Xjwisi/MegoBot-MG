...
if (!text) throw `[⚡]⌯ ضـيـف رابـط الـفـديـو\n⌯ مـثـال :\n⌯ ${usedPrefix + command} https://x.com/abdelrahman0557/status/1713591452876333547?s=20`;
if (enviando) return;
    enviando = true;
try {
 ...
 if (res?.result.type == 'video') {
     const caption = res?.result.caption ? res.result.caption : '';
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message:
          "[⚡]⌯ حــدث خــطـأ تـأكــد مــن رابــطــك",
      });
      ...
      if (response.data.code !== 200) {
        return resolve({
          status: "error",
          message: "[⚡]⌯ حــدث خــطــأ اثــنــاء الــارســال",
        });
      }

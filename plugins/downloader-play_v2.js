...
const handler = async (m, {command, usedPrefix, conn, text}) => {
  if (!text) throw `[⚡]⌯ نسيت تضيف عنوان البحث او لينك التحميل\n⌯ مــثــال :\n⌯ ${usedPrefix + command} صنع بوت واتساب عربي كامل\n⌯ ${usedPrefix + command} https://www.youtube.com/watch?v=Svb8SErvVF8&t`;
  try {
    if (command == 'اغنيه') {
      conn.reply(m.chat, `⌯ انــتــظــر مــن فـضـلـك\n⌯ يمكنك دعمي من هنا :\nhttps://solo.to/shadowsensei`, m);
      try {
        ...
        if (!aa) {
        throw new Error('[⚡]⌯ المحاولة الاولي فشلت');
       }        
      } catch {
    ...
    if (command == 'فيديو') {
      ...
      try {
        ...
        const aa_2 = await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `⌯غير مسئول عن ذنوب ما تشاهده\n▶︎ ━━━━━━━•──────────── 
      ⇆ㅤ◁ㅤ ❚❚ㅤ ▷ㅤ↻`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
        if (!aa_2) {
        throw new Error('[⚡]⌯ المحاولة الاولي فشلت');
       }
      } catch {
        const res = await fetch(`https://api.lolhuman.xyz/api/ytplay2?apikey=${lolkeysapi}&query=${text}`);
        const json = await res.json();
        await conn.sendFile(m.chat, json.result.video, 'error.mp4', `⌯ غير مسئول عن ذنوب ما تشاهده\n\n▶︎ ━━━━━━━•──────────── 
      ⇆ㅤ◁ㅤ ❚❚ㅤ ▷ㅤ↻`, m);
      }
    }
  } catch {
    throw '[⚡]⌯ حــــدث خــطــا';
  }
};
